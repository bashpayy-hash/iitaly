...
app.post("/api/portal/create", async (req, res) => {
  try {
    ...
    const { name, surname, phone, tier, profile } = req.body || {};
...
app.get("/api/portal/:code", rateLimit, (req, res) => {
  ...
  const roadmap = buildRoadmap(c.tier, c.profile);
  let total = 0, done = 0;
  for (const st of roadmap) for (const t of st.tasks) { total++; if (c.done[t.id]) done++; }

  res.json({
    ok: true,
    client: { name: c.name, surname: c.surname, tier: c.tier, createdAt: c.createdAt },
    roadmap,
    done: c.done,
